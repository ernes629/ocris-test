...
// =====================================================
// FUNCIONES DE CONTRASEÑA, HORA Y LOGS
// =====================================================
...
app.post("/api/mantenimientos", subirFotos.array("fotografias", 4), (req, res) => {
    ...
    try {
        ...
        const resultado = db.prepare(`
            INSERT INTO mantenimientos (fecha, estructura, ubicacion, tipo_mantenimiento, elemento, descripcion, observaciones, realizado_por, tiene_pendiente, pendiente, latitud, longitud, fotos, creado_en)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(fecha, estructura, ubicacion, tipo_mantenimiento, elemento, descripcion, observaciones, usuarioActual.usuario, pendienteExiste ? "Sí" : "No", pendienteFinal, latitud || null, longitud || null, fotosJSON, fechaUTC4);
...
app.get("/api/mantenimientos/:id/pdf", async (req, res) => {
    try {
        ...
        documento.moveDown(0.5);
        const descripcionLimpia = String(mantenimiento.descripcion || "Sin descripción").replace(/\r/g, "");
        documento.fillColor("#1f2937").fontSize(10).font("Helvetica").text(descripcionLimpia);
        documento.moveDown();

        documento.fillColor("#0b3d62").fontSize(14).font("Helvetica-Bold").text("OBSERVACIONES");
        documento.moveDown(0.5);
        const observacionesLimpias = String(mantenimiento.observaciones || "Sin observaciones").replace(/\r/g, "");
        documento.fillColor("#1f2937").fontSize(10).font("Helvetica").text(observacionesLimpias);
        documento.moveDown();

        const valorPend = String(mantenimiento.tiene_pendiente || "No").trim().toUpperCase();
        const existePendiente = (valorPend === "SÍ" || valorPend === "SI");
        if (existePendiente) {
            ...
            const pendienteLimpio = String(mantenimiento.pendiente || "No especificado").replace(/\r/g, "");
            documento.fillColor("#7f1d1d").fontSize(10).font("Helvetica").text(pendienteLimpio);
            documento.moveDown();
        } else {
        ...
        documento.moveDown();
        documento.fillColor("#64748b").fontSize(9).font("Helvetica").text(`Generado el ${obtenerHoraUTC4()}`, { align: "center" });
        documento.end();
    } catch (error) {
...
app.listen(PORT, () => {
    ...
    console.log("✓ Servidor Restaurado\n✓ Modo Auditoría (UTC-4)\n✓ Limpieza de PDF Activa\n================================");
});