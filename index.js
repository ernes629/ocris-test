...
// =====================================================
// FUNCIONES DE CONTRASEÑA, FECHAS Y LOGS
// =====================================================
...
function registrarLog(usuario, accion, mantenimiento_id, detalles) {
    try {
        ...
        db.prepare(`
            INSERT INTO logs_auditoria (usuario, accion, mantenimiento_id, fecha, detalles) 
            VALUES (?, ?, ?, ?, ?)
        `).run(usuario || 'Sistema', accion, mantenimiento_id, fechaUTC4, detalles);
    } catch (err) {
...
// Columnas dinámicas de diagnóstico
const columnasEquipos = [
    ["marca", "TEXT"], ["modelo", "TEXT"], ["numero_serie", "TEXT"], ["fecha_instalacion", "TEXT"],
    ["salud_porcentaje", "INTEGER DEFAULT 100"], ["criticidad", "TEXT DEFAULT 'Normal'"] 
];
...
const columnasMantenimiento = [
    ...
    ["aislamiento_ohm", "TEXT"], ["resistencia_contacto", "TEXT"], ["accion_recomendada", "TEXT"]
];
...
if (cantidadUsuarios.total === 0) {
    ...
    db.prepare(`INSERT INTO usuarios (usuario, nombre, password, rol, activo) VALUES (?, ?, ?, ?, ?)`).run("admin", "Administrador", passwordHash, "Administrador", 1);
...
app.post("/api/login", (req, res) => {
    ...
    req.session.save((err) => {
        ...
        registrarLog(usuarioDB.usuario, "LOGIN", null, "Inicio de sesión");
...
app.post("/api/mantenimientos", subirFotos.array("fotografias", 4), (req, res) => {
    const { 
        ...
        descripcion, observaciones, tiene_pendiente, pendiente, 
        latitud, longitud, aislamiento_ohm, resistencia_contacto 
    } = req.body;
    ...
    let pendienteFinal = pendienteExiste ? String(pendiente || "").trim() : null;
    ...
    try {
        ...
        const resultado = db.prepare(`
            INSERT INTO mantenimientos (
                ...
                pendiente, latitud, longitud, fotos, aislamiento_ohm, resistencia_contacto, creado_en
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(
            fecha, estructura, ubicacion, tipo_mantenimiento, elemento, 
            descripcion, observaciones, usuarioActual.usuario, pendienteExiste ? "Sí" : "No", 
            pendienteFinal, latitud || null, longitud || null, fotosJSON, 
            aislamiento_ohm || null, resistencia_contacto || null, fechaUTC4
        );
...
app.put("/api/mantenimientos/:id", subirFotos.array("fotografias", 4), (req, res) => {
    ...
    const { 
        fecha, estructura, ubicacion, tipo_mantenimiento, elemento, 
        descripcion, observaciones, tiene_pendiente, pendiente, 
        fotosRestantes, aislamiento_ohm, resistencia_contacto 
    } = req.body;
    ...
    try {
        ...
        db.prepare(`
            ...
            SET fecha = ?, estructura = ?, ubicacion = ?, tipo_mantenimiento = ?, elemento = ?, descripcion = ?, observaciones = ?, tiene_pendiente = ?, pendiente = ?, fotos = ?, aislamiento_ohm = ?, resistencia_contacto = ?
            WHERE id = ?
        `).run(fecha, estructura, ubicacion, tipo_mantenimiento, elemento, descripcion, observaciones, pendienteExiste ? "Sí" : "No", pendienteFinal, fotosJSON, aislamiento_ohm || null, resistencia_contacto || null, id);
...
app.delete("/api/mantenimientos/:id", requiereAdmin, (req, res) => {
    ...
    try {
        ...
        if (mantenimiento.fotos) {
            try {
                JSON.parse(mantenimiento.fotos).forEach(nombreFoto => {
                    const rutaFoto = path.join(carpetaFotos, nombreFoto);
...
app.get("/api/mantenimientos/:id/pdf", async (req, res) => {
    try {
        ...
        const documento = new PDFDocument({ size: "A4", margin: 45 });
        ...
        function linea(etiqueta, valor) {
            documento.font("Helvetica-Bold").text(`${etiqueta}: `, { continued: true }).font("Helvetica").text(String(valor || "No registrado"));
        }

        linea("Fecha", formatoDDMMAAAA(mantenimiento.fecha));
        ...
        documento.moveDown(0.5);
        documento.fillColor("#1f2937").fontSize(10).font("Helvetica").text(String(mantenimiento.descripcion || "").replace(/\r/g, ""));
        documento.moveDown();
        
        if (mantenimiento.aislamiento_ohm || mantenimiento.resistencia_contacto) {
            documento.fillColor("#0b3d62").fontSize(14).font("Helvetica-Bold").text("MEDICIONES TÉCNICAS");
            documento.moveDown(0.5);
            documento.fillColor("#1f2937").fontSize(10).font("Helvetica");
            if(mantenimiento.aislamiento_ohm) linea("Aislamiento", mantenimiento.aislamiento_ohm);
            if(mantenimiento.resistencia_contacto) linea("Resistencia de Contacto", mantenimiento.resistencia_contacto);
            documento.moveDown();
        }

        documento.fillColor("#0b3d62").fontSize(14).font("Helvetica-Bold").text("OBSERVACIONES");
        documento.moveDown(0.5);
        documento.fillColor("#1f2937").fontSize(10).font("Helvetica").text(String(mantenimiento.observaciones || "").replace(/\r/g, ""));
        documento.moveDown();

        const valorPend = String(mantenimiento.tiene_pendiente || "No").trim().toUpperCase();
        if (valorPend === "SÍ" || valorPend === "SI") {
            documento.roundedRect(45, documento.y, 505, 70, 6).fill("#fee2e2");
            documento.fillColor("#991b1b").fontSize(15).font("Helvetica-Bold").text("[ ! ] ATENCION: PENDIENTE");
            documento.fillColor("#7f1d1d").fontSize(10).font("Helvetica").text(String(mantenimiento.pendiente || "").replace(/\r/g, ""));
            documento.moveDown();
        } else {
            documento.fillColor("#166534").fontSize(12).font("Helvetica-Bold").text("[ OK ] MANTENIMIENTO SIN PENDIENTES");
            documento.moveDown();
        }
        ...
        if (fotos.length > 0) {
            ...
            for (let i = 0; i < fotos.length; i++) {
                ...
                if (fs.existsSync(rutaFoto)) {
                    ...
                    columnaActual = columnaActual === 0 ? 1 : 0;
                    if (columnaActual === 0) { posicionY += 200; documento.y = posicionY; }
                }
            }
        }

        documento.moveDown();
        documento.fillColor("#64748b").fontSize(9).font("Helvetica").text(`Generado el ${formatoDDMMAAAA(obtenerHoraUTC4())}`, { align: "center" });
        documento.end();
    } catch (error) {
...
// =====================================================
// LOGS, EQUIPOS Y USUARIOS
// =====================================================
app.get("/api/logs", requiereAdmin, (req, res) => {
    try {
        const logs = db.prepare(`SELECT * FROM logs_auditoria ORDER BY id DESC LIMIT 100`).all();
        res.json(logs.map(l => ({ ...l, fecha: formatoDDMMAAAA(l.fecha) })));
    } catch (error) { res.status(500).json({ ok: false, mensaje: "Error al obtener logs" }); }
});
...
app.put("/api/estructuras/:id", (req, res) => {
    const { codigo, tipo, marca, modelo, numero_serie, fecha_instalacion, ubicacion, estado, latitud, longitud, observaciones } = req.body;
    try {
        db.prepare(`UPDATE estructuras SET codigo = ?, tipo = ?, marca = ?, modelo = ?, numero_serie = ?, fecha_instalacion = ?, ubicacion = ?, estado = ?, latitud = ?, longitud = ?, observaciones = ? WHERE id = ?`).run(codigo, tipo, marca, modelo, numero_serie, fecha_instalacion, ubicacion, estado, latitud, longitud, observaciones, req.params.id);
...
app.listen(PORT, () => {
    console.log("================================\n             OCRIS\n================================");
    console.log(`Servidor SQLite activo en puerto: ${PORT}\n================================`);
    console.log(`Solo para pruebas nada Comercial Andres Ardaya\n================================`);
});console.log(`2026 probando OCRIs \n================================`);
;
