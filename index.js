...
// NUEVA RUTA: BORRADO MASIVO POR LOTE (FILTRADOS)
app.delete("/api/plan/lote", requiereAdmin, (req, res) => {
    ...
    try {
        ...
        const borrarLote = db.transaction((lista) => {
...
app.get("/api/mantenimientos/:id/pdf", async (req, res) => {
    try {
        ...
        res.setHeader("Content-Type", "application/pdf"); res.setHeader("Content-Disposition", `inline; filename="informe-${m.id}.pdf"`);
        doc.pipe(res);
        ...
        doc.fillColor("#0b3d62").fontSize(14).font("Helvetica-Bold").text("TRABAJO REALIZADO").moveDown(0.5); doc.fillColor("#1f2937").fontSize(10).font("Helvetica").text(String(m.descripcion||"")).moveDown();
        ...
        doc.fillColor("#0b3d62").fontSize(14).font("Helvetica-Bold").text("OBSERVACIONES").moveDown(0.5); doc.fillColor("#1f2937").fontSize(10).font("Helvetica").text(String(m.observaciones||"")).moveDown();
        if ((m.tiene_pendiente||"").toUpperCase() === "SÍ" || (m.tiene_pendiente||"").toUpperCase() === "SI") { doc.roundedRect(45, doc.y, 505, 70, 6).fill("#fee2e2"); doc.fillColor("#991b1b").fontSize(15).font("Helvetica-Bold").text("[ ! ] PENDIENTE"); doc.fillColor("#7f1d1d").fontSize(10).font("Helvetica").text(String(m.pendiente||"")).moveDown(); } else { doc.fillColor("#166534").fontSize(12).font("Helvetica-Bold").text("[ OK ] SIN PENDIENTES").moveDown(); }
