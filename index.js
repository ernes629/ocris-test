...
const carpetaRespaldos = path.join(__dirname, "backups_db");
...
function hacerRespaldoAutomatico() {
    try {
        ...
        fs.copyFileSync("ocris.db", archivoDestino);
...
// =====================================================
app.get("/api/plan", (req, res) => { try { res.json(db.prepare(`SELECT * FROM plan_mantenimiento ORDER BY año ASC, mes ASC, id DESC`).all()); } catch (e) { res.status(500).json({ error: "Error" }); } });
app.post("/api/plan", requiereAdmin, (req, res) => { try { const r = db.prepare(`INSERT INTO plan_mantenimiento (año, mes, placa, tipo, ejecutado_manual) VALUES (?, ?, ?, ?, ?)`).run(req.body.año, req.body.mes, req.body.placa, req.body.tipo || 'Varios', req.body.ejecutado_manual ? 1 : 0); res.json({ ok: true, id: r.lastInsertRowid }); } catch (e) { res.status(500).json({ ok: false }); } });
app.post("/api/plan/masivo", requiereAdmin, (req, res) => { try { const insert = db.prepare(`INSERT INTO plan_mantenimiento (año, mes, placa, tipo, ejecutado_manual) VALUES (?, ?, ?, ?, ?)`); const insertMany = db.transaction((lista) => { for (const eq of lista) insert.run(eq.año, eq.mes, eq.placa, eq.tipo, eq.ejecutado_manual ? 1 : 0); }); insertMany(req.body.equipos); res.json({ ok: true }); } catch (e) { res.status(500).json({ ok: false }); } });
